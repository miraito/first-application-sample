var addbattleScene_e = function(chara,enemy){
	...
	// バトル用クローン作成（player）
	var player = addplayer(chara,80,232);
	...
	// キャラの名前
	var name = labelBase( com.status.name,0,-100,'cyan' );
	// 敵の名前
	var e_name = labelBase( player.status.name,0,-100,'yellow' );
...
//=================================================戦闘計算開始
	...
	var att_skill = false;
	var def_skill = false;
	att_skill = attSkillCheck(enemy);
	def_skill = defSkillCheck(chara);
	...
	var att_skill_elements = {
		...
		add_hp : false,
		damage_change : 0,
		attack_name : com.status.attack_name
	};
	...
	if(att_skill){ // スキルがある場合は名前を取得して発動判定
		att_skill_name = get_attskill_name_by_chara(enemy);
		// スキル発動条件取得
		att_condition = get_skill_condition_by_name(att_skill_name);
		// スキル発動判定
		att_skill_flag = att_condition(enemy,chara,att_skill_elements);
	...
	if(def_skill){ // ある場合は名前を取得
		...
		// スキル発動判定
		def_skill_flag = def_condition(enemy,chara,def_skill_elements);
	}
	...
	com.status.att += att_skill_elements.att_correction;
	com.status.def += att_skill_elements.att_correction;
	att_times += att_skill_elements.times;

	// 防御側
	player.status.att += att_skill_elements.def_correction;
	player.status.def += def_skill_elements.def_correction;
	...
	if( type === 'attack'){
		...
		attack_name = com.status.equip.arm;
	}
	...
	if( type === 'attack' ){
		if( att_critical > random(100)){
			real_att = Math.floor(com.status.att * 1.2);
			critical_flag = true;
		}else{
			real_att = com.status.att;
		}
		if( real_att - player.status.def > 0 ){
			damage = real_att - player.status.def;
		}else{
			damage = 0;
		}
	}else if( type === 'spell' ){
		if( com.status.att - player.status.int > 0 ){
			damage = com.status.att - player.status.int;
		}else{
			damage = 0;
		}
	}
		...
		com.tl.delay(15)
			.waitUntil(function(){
				if( com.age % 10 === 0 ){
					...
					com.image = game.assets[com.status.image2];
					com.frame = com.status.banim[ (com.age / 10) % 3 ];
				}
				if( com.frame === com.status.banim[2] ){
					to_next();
					return true;
				}
			})
	...
	var att_cutin = function(){
		...
		var tip = game.assets[com.status.cutin_image];
		...
		.delay(15)
		.then(function(){
			skill_effect_type[att_skill_effect_no](scene,com);	
		})
		.and()
		.scaleTo(1,1,10)
		.and()
	...
	var default_set = function(){
		...
		if( att_skill_elements.add_hp === true ){
			battle_text1.tl
				...
				if( hp_e + damage < com.status.maxhp ){
					hp_e += damage;
				}else{
					hp_e = com.status.maxhp;
				}
				hp -= damage;
			})
			.delay(30)
	...
	var cut_attack = function(){
		next = false;
		if( att_skill_flag ){
			effect = action_effect_type[att_action_effect_no](scene,player,black1);
		}else{
			effect = effect_type[effect_no](scene,player,black1);
		}
		if( critical_flag ){
			scene.removeChild(name);
			battle_text1.color = 'yellow';
			battle_text1.text = 'クリティカル！！';	
			battle_text1.tl.delay(45)
			.then(function(){
				scene.addChild(name);
				default_set();
			})
